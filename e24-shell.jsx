/* N-01: five tabs in this order. The cart is not a tab (N-03). */
const NAV = [['home','Home'],['shop','Shop'],['wishlist','Wishlist'],['card',PROGRAMME],['profile','Profile']];
const GATED_TABS = ['wishlist','card','profile'];
...
const NAV_GLYPH_PATHS = {
  ...
  wishlist: ['M19.5 12.572L12 20L4.5 12.572C3.005 11.117 2.576 8.886 3.43 6.98C4.283 5.074 6.237 3.9 8.32 4.05C9.82 4.157 11.2 4.94 12 6.2C12.8 4.94 14.18 4.157 15.68 4.05C17.763 3.9 19.717 5.074 20.57 6.98C21.424 8.886 20.995 11.117 19.5 12.572Z'],
...
function Nav({ active }) {
  ...
  return (
    <nav className="nav">
      {NAV.map(([icon,label])=>(
        <a key={icon} href="#" aria-label={label} onClick={nav?(e=>{e.preventDefault();nav.tab(icon);}):undefined} aria-current={(String(active||'').toLowerCase()===icon||String(active||'').toLowerCase()===label.toLowerCase())?'page':undefined}><span className="glyph"><NavGlyph name={icon} /></span><span className="nlabel">{label}</span></a>
      ))}
    </nav>
  );
}
...
/* Session: 'guest' after "Skip for now"; anything else is a signed-in demo user. */
const Session = { isGuest(){ try { return localStorage.getItem('e24-session') === 'guest'; } catch (e) { return false; } } };
const AuthGate = { pending:null, take(){ const p = this.pending; this.pending = null; return p; } };

/* F-01 wishlist. Items are product keys from CATALOG. */
const WISH_KEY = 'e24-wishlist';
const WishStore = { keys:(()=>{ try { const s = JSON.parse(localStorage.getItem(WISH_KEY)); return Array.isArray(s) ? s : ['omega3','betacarotene','esterc']; } catch (e) { return ['omega3','betacarotene','esterc']; } })(),
  subs:new Set(), flash:null,
  save(){ try { localStorage.setItem(WISH_KEY, JSON.stringify(this.keys)); } catch (e) {} this.subs.forEach(f=>f()); },
  has(k){ return this.keys.includes(k); },
  add(k){ if (!k || this.has(k)) return; this.keys = [k].concat(this.keys); this.toast('Saved to your wishlist'); this.save(); },
  remove(k){ this.keys = this.keys.filter(x=>x!==k); this.save(); },
  toggle(k){ this.has(k) ? this.remove(k) : this.add(k); },
  toast(t){ this.flash = t; clearTimeout(this._t); this._t = setTimeout(()=>{ this.flash = null; this.subs.forEach(f=>f()); }, 2600); },
  sub(f){ this.subs.add(f); return ()=>this.subs.delete(f); } };
function useWish(){
  const [, force] = React.useState(0);
  React.useEffect(()=>WishStore.sub(()=>force(v=>v+1)), []);
  return WishStore;
}
function WishToast(){
  const w = useWish();
  return w.flash ? <div className="etoast" role="status"><span>{w.flash}</span></div> : null;
}
function HeartButton({ pkey, className = 'heartbtn', stop = true }){
  const nav = useNav();
  const w = useWish();
  const on = w.has(pkey);
  const tap = e => {
    if (stop) { e.stopPropagation(); e.preventDefault(); }
    if (Session.isGuest()) { if (nav.requireAuth) nav.requireAuth(()=>WishStore.add(pkey)); return; }
    WishStore.toggle(pkey);
  };
  return (
    <button className={className} aria-pressed={on} aria-label={on ? 'Remove from wishlist' : 'Save to wishlist'} onClick={tap}>
      <i className={'ti ti-heart' + (on ? '-filled' : '')}></i>
    </button>
  );
}

/* C-02 automatic promotions (BigCommerce). The card price never changes; the discount appears in the cart.
   Example data only: name and end date from the 24 Sep brief, the percentage is a placeholder. */
const PROMOTIONS = { vitamini:{ name:'Tjedan vitamina', until:'30. 9.', pct:10 } };
const PROMO_BY_KEY = { esterc:'vitamini', d3:'vitamini' };
const OUT_OF_STOCK = ['betacarotene'];
/* S-02: when the loyalty service is down, shopping keeps working and promotion labels are omitted. */
const LoyaltyStatus = { up:true };

/* G-12: one label component. Variants come from BigCommerce categories:
   sale (catalogue sale %), prime, gift, recommend, and promo (automatic promotion, C-02). */
function ProductLabel({ kind = 'sale', children }) {
  return <span className="plabel" data-kind={kind}>{children}</span>;
}
const promoText = pr => pr ? `Promotion: ${pr.name} · until ${pr.until}` : null;
...
/* C3 / Home rails. No points on product cards (G-04). Catalogue sale shows both prices
   and the percentage (C-01); an automatic promotion shows its label only, the price does not change (C-02). */
function ProductCard({ pkey, image, name, price, was, tag, prime, gift, recommend, promo, oos, onClick }) {
  const inCart = useCartQty(name);
  const loyaltyUp = !window.LoyaltyStatus || window.LoyaltyStatus.up;
  return (
    <div className="pcard" role="button" tabIndex={0} onClick={onClick} onKeyDown={e=>{ if (e.key === 'Enter') onClick && onClick(); }}>
      <span className="shot">
        <span className="plabels">
          {tag ? <ProductLabel kind="sale">{tag}</ProductLabel> : null}
          {prime ? <ProductLabel kind="prime">PRIME</ProductLabel> : null}
          {gift ? <ProductLabel kind="gift">Gift</ProductLabel> : null}
          {recommend ? <ProductLabel kind="recommend">Recommended</ProductLabel> : null}
        </span>
        {pkey ? <HeartButton pkey={pkey} /> : null}
      ...
      {promo && loyaltyUp ? <span className="promoline"><ProductLabel kind="promo">{promoText(promo)}</ProductLabel></span> : null}
      {oos ? <span className="oosline">Out of stock</span> : null}
    </div>
  );
}
...
Object.assign(window, { PROMOTIONS, PROMO_BY_KEY, OUT_OF_STOCK, LoyaltyStatus, Session, AuthGate, GATED_TABS, WishStore, useWish, WishToast, HeartButton, ProductLabel, promoText, SwitchList, CartStore, useCartQty, useCartCount, CartAction, CartGlyph, SearchGlyph, StatusBar, Nav, CartIcon, NavGlyph, AppIcon, AppActions, Tendril, SecLabel, ProductCard, Screen, Modal, Badge, MicroLabel, Stepper, Mechanic, Dots, NAV, NavCtx, useNav });
